...
class OmdsCommPathStatus(userAgent: String = "OlympusCameraKit", private val executeUrl : String = "http://192.168.0.10")
{
    ...
    fun getCommPathStatus(callback: IOperationCallback)
    {
        try
        {
            Log.v(TAG, " getCommPathStatus")
            val thread = Thread {
                val camInStateUrl = "$executeUrl/get_commpath.cgi"
                val response: String = http.httpGetWithHeader(camInStateUrl, headerMap, null, TIMEOUT_MS) ?: ""
                Log.v(TAG, " $camInStateUrl $response")
                val message = "path: ${pickupValue("path", response)}\n"
    ...
    companion object
    {
        private val TAG = OmdsCommPathStatus::class.java.simpleName
